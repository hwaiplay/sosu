         ...
         <!-- 디테일 카테고리 -->
            ...
            <!-- 참가비용 -->   
              <p class="mo-cost">${m.MO_COST2}원
                  <c:if test = "${sessionss eq NULL or sessionss eq m.M_IDX and m.MZ_CHECK eq 0 or m.Z_DEL_YN eq 'Y'}">
                   <form>
                   <input type="hidden" name="M_IDX"   value="${m.M_IDX}">
                   <input type="hidden" name="MO_IDX" value="${m.MO_IDX}">
                  <input type="button" id="delZzim1" name="Z_IDX" value="♡" class="heart">
					</form>                  
                  </c:if>
                  <c:if test = "${sessionss ne NULL and sessionss eq m.M_IDX and m.MZ_CHECK eq 1 and m.Z_DEL_YN eq 'N'}">
                  <input type="button" id="delZzim1" name="Z_IDX" value="♥" class="heart">
                  </c:if>
               </p>
         </div>
